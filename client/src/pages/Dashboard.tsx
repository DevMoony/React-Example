...
import { Bot, Server, Users, Zap, Trophy, MessageSquare } from "lucide-react";
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <Card className="border-t-4 border-indigo-500">
            <CardHeader className="px-4 py-5 sm:px-6 flex flex-row justify-between items-center">
              ...
              <Link href="/leaderboard" className="text-sm font-medium text-primary hover:opacity-80">
                View all
              </Link>
            </CardHeader>
            <div className="border-t border-slate-800">
              {isLoadingLeaderboard ? (
                ...
                <ul className="divide-y divide-slate-800">
                  {topUsers?.map((user) => (
                    <li key={user.id} className="px-4 py-4 sm:px-6 hover:bg-slate-900/30">
                      <div className="flex items-center">
                        <div className="w-8 h-8 flex items-center justify-center rounded-full bg-slate-800 text-slate-200 font-bold">
                          {user.rank}
                        </div>
                        <div className="ml-3 flex-1">
                          ...
                          <div className="flex items-center">
                            ...
                            <div className="ml-2 w-24 h-2 bg-slate-800 rounded-full overflow-hidden">
                              <div 
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {servers?.map((server) => (
                    <Card key={server.id} className="p-4 hover:shadow-md transition-shadow">
                      <div className="flex items-center space-x-4">
                        <div className="h-12 w-12 bg-indigo-950 rounded-full flex items-center justify-center text-indigo-400">
                          <Server className="h-6 w-6" />
                        </div>
                        <div>
                          <h3 className="text-lg font-medium">{server.name}</h3>
                          <p className="text-sm text-muted-foreground">{server.memberCount} members</p>
                        </div>
                      </div>
                      <div className="mt-4 flex flex-wrap gap-2">
                        {server.features.map((feature, index) => (
                          <span 
                            key={index} 
                            className="px-2 py-1 text-xs rounded-full bg-indigo-950 text-indigo-300"
                          >
                            {feature}
                          </span>
                        ))}
